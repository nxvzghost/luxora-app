...
**Tarefas:**
- ~~AD-007 — Webhook de recepção de mensagens do WhatsApp + controller para `ai.module.ts`~~ **CONCLUÍDA (2026-07-28)** — ver Kanban/CHANGELOG/ADR-0053.
- ~~AD-010 — Rotear `remarcar_consulta` e consulta exploratória de horários no `IntentActionRouter`~~ **CONCLUÍDA (2026-07-28)** — ver Kanban/CHANGELOG/ADR-0053.
- ~~AD-036 — Idempotência ponta-a-ponta do processamento assíncrono (Inbox Pattern), corrigindo risco Alto identificado na revisão técnica do commit AD-007/AD-010~~ **CONCLUÍDA (2026-07-29)** — ver Kanban/CHANGELOG/ADR-0054.
- AD-027 — Testes do webhook e do `WhatsAppMessageProvider` contra a API real (hoje explicitamente não testados) — **pendente**, depende de credenciais reais da Meta/WhatsApp Business API (ver Riscos).

**Critério objetivo de conclusão:**
- [x] `POST /webhooks/whatsapp` recebe mensagem real, valida assinatura, aciona `ProcessarMensagemUseCase` (assíncrono, via fila)
- [x] `IntentActionRouter` cobre 6 intents (os 4 já roteados + `remarcar_consulta` + `consultar_disponibilidade`), com `SlotNotAvailableError` tratado corretamente em reagendamento (nunca mascarado como sucesso — testado)
- [x] Processamento assíncrono é idempotente ponta-a-ponta: um retry do BullMQ nunca rechama a IA, nunca duplica ação de negócio, `Message` ou auditoria, depois da Fase 1 já ter tido sucesso (AD-036, Inbox Pattern) — provado contra BullMQ/Postgres/Redis reais.
- [ ] Teste (sandbox real ou, na ausência de sandbox, teste de contrato documentado) validando envio/recebimento **contra a API real da Meta** — pendente (AD-027), fora do alcance deste ambiente sem credenciais reais. A suíte crítica desta AD cobre a parte síncrona do webhook (handshake, HMAC, resolução de Tenant, idempotência) contra Postgres/Redis reais, mas não o envio/recebimento reais via Meta Cloud API.

**Riscos:** depende de credenciais reais da Meta/WhatsApp Business API e de rede — mesmo tipo de bloqueio de ambiente já visto no Epic 1; **AD-027 permanece parcialmente pendente até resolução externa** (conta de teste da Meta) — não bloqueia o fechamento de AD-007/AD-010, que já entregam o webhook funcional e testável dentro do que este ambiente permite.

**Resultado esperado:** um paciente real consegue interagir de ponta a ponta pelo WhatsApp, incluindo reagendar. **Alcançado para o caminho síncrono (recepção, idempotência, roteamento de intents); validação de ponta a ponta com IA/envio reais depende de AD-027.**
...
**Critério objetivo de conclusão:**
- [x] Banco, domínio, repositório, casos de uso e API implementados na ordem oficial documentada
- [x] Cenário 11 (responsável/dependente) e Cenário 12 (casal, telefone compartilhado) — cobertos por `AssociarContatoUseCase`/`ContactIntentActionRouter`, com testes unitários dedicados (fakes, não Postgres real)
- [ ] **Cenário 13 (troca de número) permanece incompleto**: `Contact.vincularAPacienteExistente()` existe no Aggregate (Fase 2) desde o início, mas nenhum Use Case/Router o chama — o eixo de classificação da Fase 6 (`PROMOVER`/`ASSOCIAR`/`DESAMBIGUAR`/`IGNORAR`/`HUMANO`) não tem um rótulo para "vincular a um Patient existente por troca de número", escopo deliberadamente deixado de fora nas Fases 6/7. Nenhum teste crítico dedicado contra Postgres real foi criado para nenhum dos 3 cenários (cobertura ficou em unitário/integração de repositório) — **item pendente, não coberto pela Fase 8** (que é hardening, não escopo funcional novo).
- [x] Colisão de nome de estado formalmente descartada com justificativa registrada (ver acima)

**Riscos:** era o maior item de escopo funcional novo do backlog — já aprovado dentro da fase de arquitetura congelada. **Risco residual aceito:** Cenário 13 sem wiring real precisa de uma AD própria antes de ser considerado coberto — registrado aqui para não ser esquecido.

**Resultado esperado:** o WhatsApp passa a reconhecer identidade de comunicação corretamente, cumprindo a visão de produto já congelada no Marco 1. **Alcançado para o gap original que motivou a AD (contato novo consegue completar o primeiro agendamento) e para os Cenários 11/12; Cenário 13 (troca de número) permanece pendente de uma AD dedicada.**
